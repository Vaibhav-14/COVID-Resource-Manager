...
<c:forEach items="${post.tags }" var="tag" varStatus="tagStatus">
				<form id="form-id" method="POST" action="/post/searchresult">
					<input type="hidden" name="${_csrf.parameterName}"
						value="${_csrf.token}" /> <input type="hidden" name="searchentry"
						value="${tag.name }"> <a href="#"
...
Comment By: <c:if test="${pageContext.request.userPrincipal.name != comment.user.username }">
...
Comment At: ${comment.dateTime }<br>
			...
			<c:if test="${isLoggedIn == true }">
				<sf:form modelAttribute="comment" action="/comment/create"
					method="post">
					<input type="hidden" name="${_csrf.parameterName}"
						value="${_csrf.token}" />
